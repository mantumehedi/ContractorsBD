...
export default function Dashboard() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (profile?.role) {
        setUserRole(profile.role as 'contractor' | 'site_manager');
      } else {
        // Default to contractor for now to ensure access
  ...
  const t = (key: string) => {
    const translations: { [key: string]: { bn: string, en: string } } = {
      ...
      show_more_days: { bn: 'আরও দেখুন (পরবর্তী 7 দিন)', en: 'Show More (Next 7 Days)' },
  ...
  const DATA_TRANSLATIONS: Record<string, { bn: string, en: string }> = {
    ...
    'Sector 7 Road': { bn: 'সেক্টর 7 রোড', en: 'Sector 7 Road' },
    ...
    '1st R.A. Bill': { bn: '1st রানিং বিল', en: '1st R.A. Bill' },
    '2nd R.A. Bill': { bn: '2nd রানিং বিল', en: '2nd R.A. Bill' },
  ...
  return (
    <main className={`min-h-screen bg-[#0F172A] text-white font-sans selection:bg-blue-500/30 flex overflow-hidden ${lang === 'bn' ? 'bn-text' : 'en-text'}`}>
      
      {/* Sidebar for Desktop */}
      <aside className="hidden lg:flex w-72 bg-[#1E293B]/20 border-r border-white/5 flex-col p-6 relative overflow-hidden backdrop-blur-xl">
        {/* Sidebar Background Glows */}
        <div className="absolute top-[-10%] left-[-10%] w-[80%] h-[40%] bg-blue-600/5 blur-[100px] rounded-full" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[80%] h-[40%] bg-emerald-600/5 blur-[100px] rounded-full" />

        <div className="relative z-10 flex flex-col h-full">
          <div className="mb-12">
            <h1 className="text-2xl font-black tracking-tighter text-white">
              CONTRACTORS<span className="text-blue-500">BD</span>
            </h1>
            <p className="text-[10px] font-bold text-white/20 uppercase tracking-[0.3em] mt-1">Management Suite</p>
          </div>
          ...
          <nav className="flex-1 space-y-2">
            {[
              ...
              { id: 'settings', icon: MoreVertical, label: t('settings') }
            ].map((item) => (
              <button
                key={item.id}
                ...
                className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 group ${
                  activeTab === item.id 
                    ? 'bg-blue-600/10 border border-blue-500/20 text-blue-400 shadow-lg shadow-blue-900/20' 
          ...
          <div className="mt-auto space-y-6">
             ...
             <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] mb-3">{lang === 'bn' ? 'ভাষা নির্বাচন' : 'Language'}</p>
                <div className="flex bg-black/20 rounded-xl p-1 border border-white/5">
                  <button 
                    onClick={() => setLang('en')}
                    className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${lang === 'en' ? 'bg-blue-600 text-white shadow-lg' : 'text-white/40 hover:text-white'}`}
                  >
                    English
                  </button>
                  <button 
                    onClick={() => setLang('bn')}
                    className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${lang === 'bn' ? 'bg-blue-600 text-white shadow-lg' : 'text-white/40 hover:text-white'}`}
                  >
                    বাংলা
                  </button>
                </div>
             ...
             <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-white/20 uppercase tracking-widest">Admin</p>
                    <p className="text-xs font-bold text-white truncate max-w-[120px]">{user?.email?.split('@')[0]}</p>
                  </div>
                </div>
                <button 
                  onClick={handleLogout}
                  className="w-full py-3.5 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-xl text-[10px] font-black uppercase tracking-widest border border-red-500/20 transition-all flex items-center justify-center gap-2"
                >
      ...
      {/* Main Content Area */}
      <div className="flex-1 h-screen overflow-y-auto relative pb-24 lg:pb-12">
        {/* Mobile Header (Hidden on LG) */}
        <header className="px-6 pt-10 pb-4 flex justify-between items-center sticky top-0 z-[50] bg-[#0F172A]/80 backdrop-blur-md border-b border-white/5 lg:hidden">
          <h1 className="text-2xl font-black tracking-tighter text-white">
            CONTRACTORS<span className="text-blue-500">BD</span>
          </h1>
            ...
            {userRole === 'contractor' && (
              <>
                <button 
                  ...
                  className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-green-400"
                >
                  <FolderPlus size={20} />
                </button>
                <button 
                  onClick={() => setShowTeamModal(true)} 
                  className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-blue-400"
                >
                  <Users size={20} />
                </button>
              </>
                  ...
                  onClick={async () => {
                    ...
                    if (profile) {
                      ...
                      if (inviteError) {
                        ...
                        // Trigger actual Supabase Auth email to "invite" them
